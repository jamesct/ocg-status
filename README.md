...
2. DevTools → Application → Cookies → 复制 `auth`（`Fe26.2**…`）的值。
...
- 三条额度：已用百分比 + 进度条（绿 <60% / 橙 60–89% / 红 ≥90%）+ 重置剩余时间
- **模型分摊**：悬浮额度行查看简表；点击展开完整分摊（模型 · $配额成本 · 占比），点窗口空白收起
- 启动即显示上次快照（`last-snapshot.json`），后台静默刷新（间隔 30–86400s 可调）
- 无边框置顶悬浮窗：可拖动、双击标题切换紧凑模式、托盘（显示/刷新/登录/设置/退出）、位置记忆、开机自启
- **外观可定制**（设置 → 外观）：跟随系统/亮/暗主题、透明度 1–100、背景（纯色/渐变调色盘、图片）、圆角、窗口尺寸
- **显示内容开关**：进度条 / 百分比 / 剩余时间 / 余额接续 / 模型分摊悬浮提示 / 模型分摊点击展开，逐个开关
- 单一设置窗口（左侧边栏：登录/刷新/外观/显示内容），非模态可与悬浮窗并行操作
